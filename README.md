...
Скрипт для REAPER: выравнивает громкость дубляжа пофразно, по EBU R128.

Скрипт находит каждую произнесённую фразу, измеряет её отдельно (K-взвешивание, R128) и приводит целиком к цели — по умолчанию −23 LUFS. Динамика **внутри** фразы не трогается: никакого райдинга, уплощения и накачки. Изменения гейна происходят только в паузах, в самой тихой их точке, где они не слышны.
...
Выделенные items; если ничего не выделено — все items на выделенных дорожках. Повторный запуск идемпотентен: анализ всегда читает исходник, а не уже обработанный сигнал.
